...
%Very very nice function: get the index of vector y, and insert to a new
%matrix - quite hard to explain
...
%Compute Cost
tmp = (-y).*log(hypothesis)-(1-y).*log(1-hypothesis);

J = ((1/m).*sum(sum(tmp))); 

% Regularized cost
Theta1_removed_bias = Theta1(:,2:end);
Theta2_removed_bias = Theta2(:,2:end);
